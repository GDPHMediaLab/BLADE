...
class BLADE(nn.Module):
    def __init__(self, x_dim, mlp_dim, patch_size):
        ...
        self.mlp = self._build_mlp(num_layers=2, input_dim=x_dim, mlp_dim=mlp_dim, output_dim=BIRADS_num_classes)
        self.bmbi_head = nn.Linear(BIRADS_num_classes, BM_num_classes)
        self.bmbi_head2 = nn.Linear(BIRADS_num_classes, BM_num_classes)
        self.init_weights()
    ...
    def forward(self, x, mask):
        ...
        og_mask = mask.clone()
        # mask = self.mask_conv3d(mask) # (N,1,D,H,W)
        # mask_conf = mask.flatten(1) # (N,L), mask[?, i] 
        # roi_mask = mask_conf.bernoulli().unsqueeze(-1) # (N,L,1), mask_bin[?, i]

        mask_pool = self.mask_max3d(og_mask) # (N,1,D,H,W)
        roi_mask = mask_pool.flatten(1).unsqueeze(-1) # (N,L,1), mask_bin[?, i]

        bg_mask = 1 - roi_mask # (N,L,1), bg_mask[?, i]
