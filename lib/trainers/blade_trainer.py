...
class BLADE_Trainer(BaseTrainer):
    r"""
    ViT 3D Trainer
    """
    ...
    def epoch_train(self, epoch, niters):
        ...
        for i, data in enumerate(train_loader):
            ...
            bi_label = data.get(args.get("BIRADS_key", "BI-RADS"), None)
            bg_bm_label = torch.zeros_like(bm_label)
            bg_bi_label = torch.zeros_like(bi_label)
            ...
            if args.gpu is not None:
                ...
                bi_label = bi_label.to(args.gpu, non_blocking=True)
                bg_bm_label = bg_bm_label.to(args.gpu, non_blocking=True)
                bg_bi_label = bg_bi_label.to(args.gpu, non_blocking=True)
            ...
            with torch.cuda.amp.autocast():
                ...
                if bi_logits is not None:
                    if args.get("bi_weight", 1.0) > 0:
                        # print("bi_weight > 0")
                        bi_loss = torch.nn.functional.cross_entropy(bi_logits, bi_label.long())
                
                        bi_cdf = torch.cumsum(bi_logits.softmax(dim=-1), dim=-1)
                        bi_continuous = 7 - bi_cdf.sum(dim=-1) # bi_cdf.sum(dim=-1) is in [1, 7], the smaller the more malignant
                        bi_loss2 = torch.nn.functional.mse_loss(bi_continuous, bi_label.float())

                        bm_conf = bm_logits.sigmoid().flatten()
                        bi2bm_conf = bi_continuous / 6.
                        bmbi_loss = torch.nn.functional.mse_loss(input=bi2bm_conf, target=bm_conf)

                        loss += bi_loss + bi_loss2 + bmbi_loss
                ...
                # if bg_bi_logits is not None:
                #     bg_bi_loss = torch.nn.functional.cross_entropy(bg_bi_logits, bg_bi_label.long())
                #     loss += bg_bi_loss
